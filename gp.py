...
class GP:
    def __init__(self, length_scale = 1., nu=2.5, noise_level=0.1):
        """
...
if __name__=='__main__':
    ...
    m = np.array(m)
    targets = None
#    targets = np.array(util.make_circle_targets(center=[35,-26]))
#    targets = np.array(util.make_circle_targets())

    gp = GP()
    skip = 5
    start = time.time()
    means, stds = gp.fit_regress(targets,m[::skip], grid_density=30, show_plot=True)
    t = time.time()-start
    s = len(m[::skip])
    print('total time;',t)
