...
if __name__=='__main__':
    ...
    try:
        ...
        #draw a square grid of 1m on the world
        minlim = -config.WINDOW_SIZE/2.
        maxlim = config.WINDOW_SIZE/2.
        very_light_grey = g.color_rgb(220,220,220)
        for x in range(-config.WINDOW_METERS/2,config.WINDOW_METERS/2):
            p1 = g.Point(x*config.PPM,minlim)
            p2 = g.Point(x*config.PPM,maxlim)
            l = g.Line(p1,p2)
            l.setFill(very_light_grey)
            l.draw(win)
        for x in range(-config.WINDOW_METERS/2,config.WINDOW_METERS/2):
            p1 = g.Point(minlim,x*config.PPM)
            p2 = g.Point(maxlim,x*config.PPM)
            l = g.Line(p1,p2)
            l.setFill(very_light_grey)
            l.draw(win)
...
###############################################################################
        if config.START_CONTROLS:
            ...
            for body in agent_bodies:
                proc = pexpect.spawn('python Agent.py '+str(body.id))
